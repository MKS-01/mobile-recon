...
func runDeviceList(cmd *cobra.Command, args []string) {
	...
	simulators, err := simctl.GetSimulators()
	if err != nil {
	...
	// Group by runtime
	byRuntime := make(map[string][]simctl.Simulator)
...
func runDeviceBoot(cmd *cobra.Command, args []string) {
	...
	sim, err := simctl.GetSimulatorByUDID(target)
	if err != nil {
		// Try to find by name
		simulators, _ := simctl.GetSimulators()
	...
	if err := simctl.BootSimulator(sim.UDID); err != nil {
...
func runDeviceShutdown(cmd *cobra.Command, args []string) {
	if len(args) == 0 {
		...
		for _, sim := range booted {
			if err := simctl.ShutdownSimulator(sim.UDID); err != nil {
...
func runDeviceInfo(cmd *cobra.Command, args []string) {
	...
	// Show architecture
	arch, _ := simctl.GetArchitecture()
	output.Info("Host Architecture: %s", arch)

	// Show installed apps count
	apps, _ := simctl.GetInstalledApps(sim.UDID)
	if len(apps) > 0 {
