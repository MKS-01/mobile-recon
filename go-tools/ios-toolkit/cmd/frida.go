...
func runFridaPs(cmd *cobra.Command, args []string) {
	...
	if fridaPsPath != "" {
		...
		cmdOutput, err := simctl.ExecuteCommand(fridaPsPath, "-D", sim.UDID)
		if err != nil {
			// Fall back to listing all local processes (which includes simulator apps)
			cmdOutput, err = simctl.ExecuteCommand(fridaPsPath, "-a")
			if err != nil {
	...
	apps, err := simctl.GetInstalledApps(sim.UDID)
	if err != nil {
...
func runFridaKill(cmd *cobra.Command, args []string) {
	...
	if err := simctl.TerminateApp(sim.UDID, target); err != nil {
...
func getFridaVersion() (string, error) {
	...
	output, err := simctl.ExecuteCommand(fridaPath, "--version")
	if err != nil {
		return "", err
	}
...
func testFridaConnectivity() bool {
	...
	_, err := simctl.ExecuteCommand(fridaPsPath, "-a")
	return err == nil
}
